...

{
  ...
  if (!Via.finalizationRegistry) console.warn('[Via.js] No WeakRefs support - will leak memory');
  ...
  function FinalizeID(id) {
    ...
    if (finalizeTimerId === -1) finalizeTimerId = setTimeout(CleanupIDs, finalizeIntervalMs);
  }
  ...
  function OnCallback(data) {
    ...
    if (!func) throw new Error('invalid callback id');
  ...
  self.get = function (proxy) {
    if (typeof proxy === 'function') {
      ...
      if (typeof objectId === 'number') return AddGet(objectId, null); // null path will return object itself (e.g. in case it's a primitive)

      // Identify Via property proxy by testing if its target symbol returns anything
      const target = proxy[Via.__TargetSymbol];
      if (target) return AddGet(target._objectId, target._path);
    }
